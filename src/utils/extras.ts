...
export function additionalProps(client: SplayBot) {
    ...
    return {
        ...
        async createChannelSetup(Schema: any, channel: Channel, interaction: CommandInteraction | SimpleCommandMessage) {
            ...
            client.extras.embed(
                {
                    title: 'Successful!',
                    desc: `Channel has been set up successfully! \n **[To learn how to use me read my documentation](https://splay.gitbook.io/splay)**`,
                    fields: [
                        {
                            name: `<:Discord_Channel:1122703272608796672> Channel`,
            ...
            client.extras.embed(
                {
                    ...
                    fields: [
                        {
                            name: `<:member:1133416367060684820> Role`,
        ...
        async getChannelTicket(interaction: CommandInteraction | SimpleCommandMessage) {
            const ticketChannelData = await ticketChannels.findOne({
                Guild: interaction instanceof CommandInteraction ? interaction.guildId : interaction.message.guildId,
                channelID: `${interaction instanceof CommandInteraction ? interaction.channelId : interaction.message.channelId
                    }`,
            });
        ...
        async transcript(client: SplayBot, channel: TextChannel) {
            const file = await createTranscript(channel, {
                footerText: 'Transcript by Splay',
                poweredBy: false,
            });
        ...
        async fetchLevels(userId: string, guildId: string, fetchPosition = true) {
            ...
            if (fetchPosition === true) {
                ...
                userReturn.position = leaderboard.findIndex((i: { User: string; }) => i.User === `${userId}`) + 1;
            }
